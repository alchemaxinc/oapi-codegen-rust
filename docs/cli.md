...
* `--install-deps` — After writing, run `cargo add` for each crate the generated code needs.
   ...
   If set, this runs without prompting. If not set, and stdin is an interactive terminal, you are asked first; otherwise the run only prints the dependency list. When the nearest `Cargo.toml` can be read, only crates missing from it are reported and `cargo add` targets that manifest.
