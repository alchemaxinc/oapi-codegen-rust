...
impl Client {
    ...
    pub fn get_form(&self) -> Result<GetFormResponse, ClientError> {
        let url = format!("{}/form", self.base_url);
        let response = self.http.request(reqwest::Method::GET, url).send()?;
        let status = response.status();
        if status.as_u16() == 200 {
            let text = response.text()?;
            let body: Form = serde_urlencoded::from_str(&text)
                .map_err(|error| return ClientError::Decode(error.to_string()))?;
            return Ok(GetFormResponse::Ok(body));
        }
