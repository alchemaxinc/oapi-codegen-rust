...
impl Client {
    ...
    pub fn create_thing(
        &self,
        body: CreateThingRequestBody,
    ) -> Result<CreateThingResponse, ClientError> {
        let url = format!("{}/things", self.base_url);
        let mut request = self.http.request(reqwest::Method::POST, url);
        match body {
            CreateThingRequestBody::Json(value) => {
                request = request.json(&value);
            }
            CreateThingRequestBody::Form(value) => {
        ...
        if status.as_u16() == 204 {
            return Ok(CreateThingResponse::NoContent);
        }
